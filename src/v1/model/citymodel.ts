...
export class dbCity extends appdb {
  ...
  async checkCity(city: string) {
    ...
    try {
      ...
      const cities = await this.listRecords("*");
  ...
  async insertCity(city: string, state: string, createdIp: string) {
    ...
    try {
      ...
      let existingCity: any[] = await this.listRecords("*");
  ...
  async deleteCity(id: number) {
    ...
    try {
      ...
      let deleteResult = await this.update(
        this.table,
        updateData,
        ` WHERE id = '${id}' AND is_deleted=false`
      );
  ...
  async getCityDetails(
    ...
    const return_data = {
      pickupCityDetails: null as {
        id: number;
        city_name: string;
        city_state_name: string;
      } | null,
      dropCityDetails: null as {
        id: number;
        city_name: string;
        city_state_name: string;
      } | null,
    ...
    try {
      let cityResult: any = await this.select(
        this.table,
        "id, city_name, city_state_name",
        `WHERE city_name IN ('${pickupCity}', '${dropCity}')`,
        "",
        ""
      );

      // console.log("City Result:", cityResult); // Debugging
      ...
      return_data.pickupCityDetails =
        cityResult.find((city: any) => city.city_name === pickupCity) || null;

      return_data.dropCityDetails =
        cityResult.find((city: any) => city.city_name === dropCity) || null;

      if (!return_data.pickupCityDetails || !return_data.dropCityDetails) {
